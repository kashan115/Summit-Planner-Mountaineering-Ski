...
def plan_trip(destination, activity):
    model = genai.GenerativeModel('gemini-2.5-flash')
    
    prompt = f"""
    Act as a mountain guide. Create a detailed {activity} plan for {destination}.
    Include: Route info, Weather/Avalanche resource links (NOAA, NWAC), 
    Driving/Trailhead details, Permits, and a Gear list.
...
if __name__ == "__main__":
    plan_trip("Mount Rainier", "Mountaineering")`;
...
curl http://localhost:11434/api/generate -d '{
  "model": "llama3",
  "stream": false,
  "prompt": "Act as a mountain guide. Create a JSON plan for climbing Mount Rainier including route, gear, and permit details."
}'`;

  const reactCode = `# Run this Web App Locally

# 1. Clone the repository
git clone https://github.com/your-username/summit-planner-ai.git
